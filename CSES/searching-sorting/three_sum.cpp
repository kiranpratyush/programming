...
{
    auto start_value = array[i];
    auto required_sum = target_sum-(start_value.first);
    ll start_pointer =i+1;
    ...
    while(start_pointer<end_pointer)
    {
        ...
        if(sum==required_sum)
        {
            printf("%lld %lld %lld",start_value.second+1,array[start_pointer].second+1,array[end_pointer].second+1);
            return 0;
        }
